...
var config = {
	...
	*/
	overlays: [
		...
		{
			group: 'Amenities/об\'єкта',
			title: 'Lawyers for refugees/Адвокати для біженців',
			query: '(node[office=lawyer]["fee:conditional"="no_@_refugees"]({{bbox}});node(w);way[office=lawyer]["fee:conditional"="no_@_refugees"]({{bbox}});node(w);relation[office=lawyer]["fee:conditional"="no_@_refugees"]({{bbox}});node(w);node[office=lawyer]["fee"="no"]({{bbox}});node(w);way[office=lawyer]["fee"="no"]({{bbox}});node(w);relation[office=lawyer]["fee"="no"]({{bbox}});node(w););out meta;',
			iconSrc: imgSrc + 'icones/symbols/office/lawyer.svg',
			iconStyle: 'background-color:#ffFFFF',
			style: function (feature) {
				var key_regex = /^name:uk$/
				var name_key = feature.getKeys().filter(function(t){return t.match(key_regex)}).pop() || "name"
		...
		{
			group: 'Amenities/об\'єкта',
			title: 'Bus station/Автобусна зупинка',
			query: '(node[amenity=bus_station]({{bbox}});node(w);way[amenity=bus_station]({{bbox}});node(w);relation[amenity=bus_station]({{bbox}});node(w););out meta;',
			iconSrc: imgSrc + 'icones/symbols/amenity/bus_station.svg',
			iconStyle: 'background-color:#ffFFFF',
			style: function (feature) {
				var key_regex = /^name:uk$/
				var name_key = feature.getKeys().filter(function(t){return t.match(key_regex)}).pop() || "name"
		...
		{
			group: 'Dopomoha.pl data',
			title: 'Dopomoha OSMData<br> (from <a href="https://www.dopomoha.pl">Dopomoha.pl</a>)',
...
var name = feature.get(name_key) || '';
		...
		{
			group: 'Dopomoha.pl data',
			title: 'Dopomoha Zbiorki<br> (from <a href="https://www.dopomoha.pl">Dopomoha.pl</a>)',
